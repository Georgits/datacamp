...
df = pd.read_csv("weather_data_austin_2010.csv", header = 0, index_col=3, parse_dates = ['Date'])
...
# Plot the CDF
df.fraction.plot(kind='hist', normed=True, bins=30, cumulative=True, range=(0,.3))
plt.show()
...
ts1 = ts0.loc['2010-10-11 21:00:00':'2010-10-11 21:59:59']
...
# The first, ts1, includes weekends, but the second, ts2, does not. 
...
df = pd.read_csv("austin_airport_departure_data_2015_july.csv", header = 10, index_col='Date (MM/DD/YYYY)', parse_dates = True)
...
# The pre-loaded data ts1 has value for all times, yet the data set ts2 does not: it is missing data for the weekends. 
...
# df = df.loc['2010-feb','Temperature']
